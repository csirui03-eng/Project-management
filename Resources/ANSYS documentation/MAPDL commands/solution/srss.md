...
method: srss
group: solution
source: PyMAPDL 0.74.1 docstrings, pandoc 3.9.0.2, ansys.mapdl.core._commands.solution.spectrum_options.SpectrumOptions.srss
...
PyMAPDL: `mapdl.srss(signif='', label='', abssumkey='', forcetype='', **kwargs)`
...
Absolute Sum combination key (for [[spopt|SPOPT]],MPRS only):

- `NO` - Do not use the Absolute Sum method (default).

- `YES` - Combine the modes per excitation direction using the Absolute Sum method, then combine the resulting quantities using the square root of sum of squares method.

  When using Absolute Sum combination, the excitation direction must be specified using the [[sed|SED]] command.
...
Ansys help: https://ansyshelp.ansys.com/Views/Secured/corp/v232/en//ans_cmd/Hlp_C_SRSS.html
