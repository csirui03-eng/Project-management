...
\* `par : str` - Specifies the parameter or sets of parameters listed. For array parameters, use `IMIN`, `IMAX`, etc. to specify ranges. Use [[dim|*DIM]] to define array parameters. Use `*VEDIT` to review array parameters interactively. Use [[vwrite|*VWRITE]] to print array values in a formatted output. If `Par` is blank, list all scalar parameter values, array parameter dimensions, and abbreviations. If ARGX, list the active set of local macro parameters (ARG1 to ARG9 and AR10 to AR99) ( [[use|*USE]] ).
...
- `imin, imax, jmin, jmax, kmin, kmax, lmin, lmax, mmin, mmax : str` - Range of array elements to display (in terms of the dimensions (row, column, plane, book, and shelf). Minimum values default to

  1\. Maximum values default to the maximum dimension values. Zero may be input for `IMIN`, `JMIN`, and `KMIN` to display the index numbers. See [[taxis|*TAXIS]] command to list index numbers of 4- and 5-D tables.
