...
## Deletion test for modifiers (flag, do not delete)

For each adjective and adverb, delete it mentally and reread the sentence. If nothing checkable changed, flag the word for the author. Do not remove it yourself: some modifiers earn their place through rhythm, emphasis, or a distinction the test cannot see, and that call belongs to human judgement. A flagged modifier is a question, not a defect.

- FLAG: "specific failures in the Section 1 work" (delete "specific": the sentence loses nothing checkable)
- FLAG: "various approaches have been proposed" (delete "various": same set of approaches)
- KEEP without flagging: "single-leaf partition" (deleting changes the physics), "grew just 3.6%" (deleting removes the interpretation the qualifier carries)

Common flag candidates: specific, particular, certain, various, key, overall, general, actual. These pass the banned-word lists while doing the same nothing; the test catches the construction, not the vocabulary.
...
10. Run the deletion test on adjectives and adverbs; list flagged modifiers for the author with their locations. Flag only; do not remove.
11. Read it aloud. If a phrase would sound unnatural to a colleague, rewrite it.
